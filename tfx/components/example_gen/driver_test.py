...
class DriverTest(tf.test.TestCase):
  ...
  def testRun(self):
    ...
    pipeline_node.parameters.parameters[
        utils.INPUT_BASE_KEY].field_value.string_value = (
            self._input_base_path)
    pipeline_node.parameters.parameters[
        utils.INPUT_CONFIG_KEY].field_value.string_value = (
            json_format.MessageToJson(
                example_gen_pb2.Input(splits=[
                    example_gen_pb2.Input.Split(
                        name='s1',
    ...
    # Will verify that this uri is not changed.
    example.uri = 'my_uri'
    output_dic = {utils.EXAMPLES_KEY: [example]}
    result = ir_driver.run(None, output_dic, None)
    print(result)
    ...
    self.assertEqual(
        output_example.custom_properties[utils.VERSION_PROPERTY_NAME].int_value,
        1)
    self.assertRegex(
